...
def compute_prediction_loss(
    ...
    reg_target = boxes_encoded.view(N * A * S, -1)[fg_mask]  # (n, 19)
    reg_pred = reg_pred.view(N * A * S, -1)[fg_mask]  # (n, 30)
...
def model_fn(model, batch_dict, target_mode, disentangled_loss, reduce_batch=False):
    ...
    if 'val_time' in batch_dict:
        time_start = time.time()
    net_pred = model(net_input)  # (N, C)  (39002,3)==(39002,31)
    if 'val_time' in batch_dict:
        batch_dict['val_time'] = time.time() - time_start
        # 2023-06-29 Jinzheng Guang
...
def model_eval_collate_fn(
    ...
    if nuscenes:
        ...
        try:
            ap_dict = eval_jrdb(
                gt_dir="data/JRDB2019/train_dataset_with_activity/labels_kitti",
...
def write_nuscenes_results(
    ...
    for boxes, scores, cls_idx in zip(boxes_all, scores_all, cls_all):
        ...
        # ]
        # 2023-04-13 Jinzheng Guang
        for box, score in zip(boxes_raw, scores):
            temp = ub3d.box_to_nuscenes(box, score, sample_token, detection_name)
            if all(np.array(temp['size']) > 0) and temp['detection_score'] > 1e-2:
                sr_list += [temp]
